...
class Brand(BaseModel):
    ...
    discount = models.OneToOneField(
        to=Discount, on_delete=models.PROTECT, null=True, blank=True)
...
class Category(BaseModel):
    ...
    parent = models.ForeignKey('self', on_delete=models.SET_NULL,
                               verbose_name='Parent Category', null=True, blank=True)
    image = models.FileField(null=True, default=None,
                             upload_to='category/', blank=True)
...
class Comment(BaseModel):
    ...
    content = models.CharField(
        max_length=250, null=True, default='So far so good', help_text="Comment text")
...
class Product(BaseModel):
    ...
    comment = models.ForeignKey(
        to=Comment, on_delete=PROTECT, null=True, blank=True)
    name = models.CharField(max_length=50, default='',
                            help_text="Name of product")
    image = models.FileField(null=True, default=None,
                             upload_to='product/', blank=True)
    price = models.PositiveIntegerField(
        default=0, null=False, help_text="Price of product")
    description = models.CharField(
        max_length=250, help_text="Product description", default="description")
    count = models.PositiveIntegerField(
        default=1, null=False, help_text="Count of product")
