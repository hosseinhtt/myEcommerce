...
class Coupon(BaseDiscount):
    ...
    code = models.CharField(
        max_length=50, help_text='Coupon Code', unique=True)
...
class Status(BaseModel):
    ...
    title = models.CharField(max_length=10,
                             choices=[('new', 'New'), ('check', 'Check Out'),
...
class Order(BaseModel):
    ...
    coupon = models.OneToOneField(
        to=Coupon, on_delete=models.PROTECT, null=True, blank=True)
    total_price = models.PositiveIntegerField(
        default=0, help_text='Total price of the order')
    final_price = models.PositiveIntegerField(
        default=0, help_text='Final price after using coupon code')
    address = models.ForeignKey(
        to=Address, on_delete=models.PROTECT, null=True, blank=True)
...
class OrderItem(BaseModel):
    ...
    count = models.PositiveIntegerField(
        default=1, help_text='Count of order items')
